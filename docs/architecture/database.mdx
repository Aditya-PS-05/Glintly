...
description: 'Database schema, models, and data management in Glintly'
---

## Overview

Glintly uses PostgreSQL as its primary database with Prisma as the ORM. The database schema is designed to support real-time collaborative code generation, message history, and code fragment management.

## Database Schema

The core database schema consists of three main entities that work together to provide the Glintly experience:
...
# Create daily backup
DATE=$(date +%Y%m%d)
pg_dump glintly > backups/glintly_$DATE.sql

# Compress backup
gzip backups/glintly_$DATE.sql

# Upload to cloud storage
aws s3 cp backups/glintly_$DATE.sql.gz s3://glintly-backups/
```

```bash Recovery
# Restore from backup
psql glintly < backups/glintly_20231201.sql
